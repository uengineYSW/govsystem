        ...
        <v-card-text style="margin-top:-30px;">
            <v-row no-gutters style="margin: 15px 0px -30px -15px;">
                <v-col style="max-width:140px;">
                    <String class="attributes-list" label="ApplicationId" v-model="value.parameters.applicationId" :editMode="editMode"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        name: 'ServiceApplicationFormMapping',
        components:{},
        props: {},
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'serviceApplications/search/findByServiceApplicationFormMapping',
                parameters: {}
            },
        }),
        created() {
