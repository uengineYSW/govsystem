<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>ApplicationDocumentId</v-list-item-title>
                    </v-list-item-content>
                </template>
        ...
        <ApplicationDocumentId v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import ApplicationDocumentId from '../ApplicationDocumentId.vue';

    export default {
        name: 'ApplicationDocumentIdManager',
        components: {
            ApplicationDocumentId
        },
        props: {
