<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>PdfProcessingInfo</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            ...
                            EncryptionLevel :  {{item.encryptionLevel }} <br>
                            OutputControl :  {{item.outputControl }} <br>
                            WatermarkPosition :  {{item.watermarkPosition }} <br>
        ...
        <PdfProcessingInfo v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import PdfProcessingInfo from '../PdfProcessingInfo.vue';

    export default {
        name: 'PdfProcessingInfoManager',
        components: {
            PdfProcessingInfo
        },
        props: {
